...
ros::init(argc, argv, "asg1_minimal_controller"); //name this node 
    ...
    ros::Publisher my_publisher_object = nh.advertise<std_msgs::Float64>("asg1_cmd", 1);
    double Kv = 1.0; // velocity feedback gain 
    double dt_controller = 0.1; //specify 10Hz controller sample rate (pretty slow, but 
    //illustrative) 
    double sample_rate = 100000.0 / dt_controller; // compute the corresponding update frequency 
