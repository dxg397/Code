...
class LogisticRegression(object):
    ...
    def fit(self, samples, labels, sample_weights = np.ones(labels.shape[0])):
        """
    ...
    def gradient(self, samples, labels,sample_weights):
        ...
        for i, X in enumerate(samples.astype(np.float64)):
            ...
            derivative_of_weights = sample_weights[i] * derivative_of_weights
            derivative_of_bias = sample_weights[i] * derivative_of_bias

        derivative_of_weights += self._lambda * np.linalg.norm(self.weights) / 2
