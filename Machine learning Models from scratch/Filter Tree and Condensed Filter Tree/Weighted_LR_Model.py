...
class Weighted_LogisticRegression(object):
    ...
    def fit(self, samples, labels, sample_weights):
        ...
        for i in range(self.num_iter):
            gradient_weights, gradient_bias = self.gradient(samples, labels,sample_weights)
    ...
    def predict(self, x):
        ...
        return log_probab > 0, confidence
    ...
    def gradient(self, samples, labels,sample_weights):
        """
        ...
        for i, X in enumerate(samples.astype(np.float64)):
            ...
            if labels[i] == True:
                derivative_of_weights += (cond_likelihood - 1) * X * sample_weights[i]*C
                derivative_of_bias += (cond_likelihood - 1) * sample_weights[i] *C
            else:
                derivative_of_weights += cond_likelihood * X * sample_weights[i] *C
                derivative_of_bias += cond_likelihood * sample_weights[i] *C
